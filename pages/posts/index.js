...
export default function Post({ posts, loading }) {
  ...
  return (
    <>
      <article className={styles.addto}>
            ...
            {posts.map((country, i) => (
              <div className={styles.card} key={i}>
                    <div className={styles.flag}>
                      <Image src={`https://flagcdn.com/${getCountryCode(country)}.svg`} width={300} height={180} layout="responsive" />
                    </div>
      ...
      <div className="return-block">
          <Link href="/">
              <button className="back-btn">Return to Home</button>
          </Link>
        </div>
    </>
  );
